...
// glyphcade — Minesweeper: the board model, and nothing else.
//
...
// The only glyphcade header this file includes, and still no termforge one — which
...
// The PRNG moved to <glyphcade/arcade/rng.hpp> when 2048 needed the same
// generator, and this alias is why nothing here had to change: `Rng`,
// `minesweeper::Rng` and `glyphcade::Rng` are all the same type.
//
