...
TEST_CASE("the project library is linked and reachable", "[bootstrap][smoke]") {
  REQUIRE(std::string_view{PROGRAM_NAME} == "glyphcade");

  const auto* v = glyphcade::version_string();
  REQUIRE(v != nullptr);
...
TEST_CASE("build_has_audio agrees with the CMake option",
          "[bootstrap][audio]") {
  // GLYPHCADE_EXPECT_AUDIO comes from CMake via this dir's CMakeLists.txt;
  // build_has_audio() comes from the library, which was compiled with (or
  // without) a PRIVATE GLYPHCADE_WITH_AUDIO. Two independent paths from one
  ...
  // -DGLYPHCADE_WITH_AUDIO=OFF. The second is the configuration CI runs.
  REQUIRE(glyphcade::build_has_audio() == (GLYPHCADE_EXPECT_AUDIO != 0));
}
