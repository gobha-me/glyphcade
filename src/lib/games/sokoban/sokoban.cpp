...
// exists.
//
// ── What MapWidget made awkward for its first consumer ──────────────────────
//
...
// it. All four were found on v0.2.2 (and still existed in v0.5.1).
//
//  1. set_map_size() wiped every layer while claiming to preserve the overlap.
//     Fixed by termforge #127; load() still naturally sizes before populating.
//  2. Mouse picking forced the app to reproduce the widget's camera and
//     floored-viewport arithmetic. Fixed by termforge #128; handle_mouse() now
//     delegates to MapWidget::tile_at().
...
auto Sokoban::handle_mouse(const termforge::MouseEvent& mouse) -> bool {
  ...
  const auto tile = m_map.tile_at(mouse.x, mouse.y);
  if (!tile) return false;
  const auto [tx, ty] = *tile;
